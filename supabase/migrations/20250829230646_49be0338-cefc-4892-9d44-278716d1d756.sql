...
ALTER TABLE maity.sessions ENABLE ROW LEVEL SECURITY;
...
-- RLS Policies for maity.coaching_sessions (connected via session_id -> sessions.user_id)
CREATE POLICY "Platform admins can view all coaching sessions" 
ON maity.coaching_sessions FOR SELECT 
USING (public.get_user_role() = 'platform_admin');

CREATE POLICY "Org admins can view sessions from their company users" 
ON maity.coaching_sessions FOR SELECT 
USING (
  public.get_user_role() = 'org_admin' 
  AND EXISTS (
    SELECT 1 FROM maity.sessions s
    JOIN maity.users u ON s.user_id = u.id
    WHERE s.id = maity.coaching_sessions.session_id 
...
-- RLS Policies for maity.sessions
CREATE POLICY "Platform admins can view all sessions" 
ON maity.sessions FOR SELECT 
USING (public.get_user_role() = 'platform_admin');
...
USING (
  ...
  AND EXISTS (
    ...
    WHERE u.id = maity.sessions.user_id 
    AND u.company_id = public.get_user_company_id()
  )
);
...
-- RLS Policies for maity.plans (connected via session_id -> sessions.user_id)
CREATE POLICY "Platform admins can view all plans" 
ON maity.plans FOR SELECT 
USING (public.get_user_role() = 'platform_admin');

CREATE POLICY "Org admins can view plans from their company users" 
ON maity.plans FOR SELECT 
USING (
  public.get_user_role() = 'org_admin' 
  AND EXISTS (
    SELECT 1 FROM maity.sessions s
    JOIN maity.users u ON s.user_id = u.id
    WHERE s.id = maity.plans.session_id 
    AND u.company_id = public.get_user_company_id()
  )
);
