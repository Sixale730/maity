-- =====================================================================
-- Add agent_id to get_or_create_user_progress
-- Purpose: Return agent_id for scenario-specific agent selection
-- Date: 2025-11-21
-- =====================================================================
...
BEGIN
  ...
  SELECT
    ...
    vs.agent_id as scenario_agent_id,
    vps.is_locked,
  ...
  WHERE vup.id = v_progress_id
    ...
    AND vs.agent_id IS NOT NULL  -- Only return scenarios with agent_id configured
  LIMIT 1;
END;
$$;

-- Create public wrapper with agent_id
...
COMMENT ON FUNCTION maity.get_or_create_user_progress IS 'Get or create user progress for a profile with complete scenario information including agent_id. Only returns scenarios with agent_id configured.';
