-- Fix get_or_create_user_progress VARCHAR type mismatches
-- Problem: Function declares generic VARCHAR but columns have specific lengths
-- This causes PostgreSQL error: "Returned type character varying(50) does not match expected type text"
-- Solution: Use TEXT type which is compatible with VARCHAR of any length in PostgreSQL
...
RETURNS TABLE(
  ...
  current_scenario_name TEXT,  -- Changed to TEXT for compatibility with VARCHAR(100)
  current_scenario_code TEXT,   -- Changed to TEXT for compatibility with VARCHAR(50)
  is_locked BOOLEAN,
  ...
  difficulty_name TEXT,        -- Changed to TEXT for compatibility with VARCHAR(100)
  difficulty_mood TEXT,         -- Changed to TEXT for compatibility with VARCHAR(50)
...
-- Create public wrapper with corrected types
...
COMMENT ON FUNCTION maity.get_or_create_user_progress IS 'Get or create user progress for a profile with complete scenario information - FIXED using TEXT types for VARCHAR compatibility';
