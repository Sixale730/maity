-- Migration: Add outfit_preset column to avatar_configurations
-- This allows users to select outfit presets (casual, business, worker, formal, sporty)

-- Add outfit_preset column with default 'casual' for backwards compatibility
ALTER TABLE maity.avatar_configurations
ADD COLUMN IF NOT EXISTS outfit_preset TEXT NOT NULL DEFAULT 'casual';

-- Add check constraint for valid preset values (drop first if exists)
ALTER TABLE maity.avatar_configurations
DROP CONSTRAINT IF EXISTS avatar_outfit_preset_check;

ALTER TABLE maity.avatar_configurations
ADD CONSTRAINT avatar_outfit_preset_check
CHECK (outfit_preset IN ('casual', 'business', 'worker', 'formal', 'sporty'));

-- Create index for faster lookups by outfit
CREATE INDEX IF NOT EXISTS idx_avatar_configurations_outfit_preset
ON maity.avatar_configurations(outfit_preset);
...
DROP FUNCTION IF EXISTS public.upsert_avatar_configuration(UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB, JSONB);
DROP FUNCTION IF EXISTS maity.upsert_avatar_configuration(UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB, JSONB);

-- Update RPC function to include outfit_preset
...
-- Update upsert function to handle outfit_preset
...
GRANT EXECUTE ON FUNCTION public.upsert_avatar_configuration(UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB, JSONB) TO authenticated;

COMMENT ON COLUMN maity.avatar_configurations.outfit_preset IS 'Selected outfit preset: casual, business, worker, formal, sporty';
