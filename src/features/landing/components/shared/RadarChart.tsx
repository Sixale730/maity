...
function getPolygonPoints(count: number, radius: number): { x: number; y: number }[] {
  ...
  return (
    <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`} className="drop-shadow-lg">
      ...
      {GRID_LEVELS.map((level, idx) => {
        ...
        return (
          <text key={`label-level-${idx}`} x={pos.x + 4} y={pos.y - 2} fill="rgba(255,255,255,0.3)" fontSize={7} fontFamily="sans-serif">
            {GRID_LABELS[idx]}
          </text>
        );
      })}
      ...
      {data.map((_, i) => {
        ...
        return (
          <line key={`axis-${i}`} x1={CENTER} y1={CENTER} x2={end.x} y2={end.y} stroke="rgba(255,255,255,0.08)" strokeWidth={0.5} />
        );
      })}

      <polygon points={pointsToString(dataPoints)} fill={color} fillOpacity={0.2} stroke={color} strokeWidth={1.5} />
      ...
      {data.map((d, i) => {
        ...
        return (
          <text key={`label-${i}`} x={pos.x} y={pos.y} fill="rgba(255,255,255,0.7)" fontSize={8} fontFamily="sans-serif" textAnchor="middle" dominantBaseline="central">
            {d.name}
          </text>
        );
      })}
