...
import { Sparkles, ChevronRight, Share2, Check, TrendingUp } from 'lucide-react';
import { FadeIn } from '../shared/FadeIn';
import { RadarChart } from '../shared/RadarChart';
import { LANDING_COLORS } from '../../constants/colors';
import { PRODUCT_ARCHETYPES, PRODUCT_QUIZ_QUESTIONS } from '../../constants/quiz-data';
...
function getResult(answers: string[]): QuizResult {
  ...
  const percentages = sorted.map(([key, count]) => ({
    key,
    name: PRODUCT_ARCHETYPES[key].name,
    pct: Math.round((count / total) * 100),
  }));

  // Fill in any archetype with 0 answers
  Object.keys(PRODUCT_ARCHETYPES).forEach((k) => {
    if (!percentages.find((p) => p.key === k)) {
      percentages.push({ key: k, name: PRODUCT_ARCHETYPES[k].name, pct: 0 });
    }
  });

  const winnerKey = sorted[0][0];
  return { archetype: PRODUCT_ARCHETYPES[winnerKey], key: winnerKey, percentages };
}

export const ArchetypeQuiz = () => {
  ...
  const handleAnswer = (key: string) => {
    ...
    setAnswers(next);
    if (currentQ + 1 < PRODUCT_QUIZ_QUESTIONS.length) {
  ...
  const handleShare = async () => {
    ...
    try {
      await navigator.share({
        title: `Soy un ${result.archetype.name}`,
        text: `${result.archetype.tagline} - Descubre tu superpoder en Maity`,
        url: window.location.href,
      });
    } catch { /* user cancelled */ }
  };

  const q = PRODUCT_QUIZ_QUESTIONS[currentQ];
  const progress = ((currentQ + 1) / PRODUCT_QUIZ_QUESTIONS.length) * 100;
    ...
    return (
      <section className="min-h-screen flex items-center justify-center px-6 py-24">
        <FadeIn className="max-w-xl text-center">
          ...
          >
            <Sparkles className="w-4 h-4" style={{ color: LANDING_COLORS.maityPink }} />
          ...
          <h1 className="text-3xl md:text-5xl font-bold mb-4" style={{ color: LANDING_COLORS.textMain }}>
            ¿Cuál es tu{' '}
            <span className="bg-clip-text text-transparent" style={{ backgroundImage: `linear-gradient(135deg, ${LANDING_COLORS.maityPink}, ${LANDING_COLORS.maityBlue})` }}>
              superpoder
            </span>{' '}
            de comunicación?
          </h1>
          <p className="text-lg mb-8" style={{ color: LANDING_COLORS.textMuted }}>
            7 preguntas rápidas para descubrir tu estilo comunicativo dominante y cómo potenciarlo.
          </p>
          <button
      ...
      <section className="min-h-screen flex items-center justify-center px-6">
        <div className="text-center">
          <div
            ...
            style={{ borderColor: `${LANDING_COLORS.maityPink} transparent ${LANDING_COLORS.maityPink} ${LANDING_COLORS.maityPink}` }}
          />
          <p className="text-lg" style={{ color: LANDING_COLORS.textMuted }}>Analizando tu perfil...</p>
        </div>
      </section>
    );
  }
        ...
        <div className="max-w-2xl w-full">
          ...
          <div className="h-1.5 rounded-full bg-white/10 mb-6 overflow-hidden">
            <div
              ...
              style={{ width: `${progress}%`, backgroundImage: `linear-gradient(90deg, ${LANDING_COLORS.maityPink}, ${LANDING_COLORS.maityBlue})` }}
            />
          </div>
          <p className="text-sm mb-6" style={{ color: LANDING_COLORS.textMuted }}>
            Pregunta {currentQ + 1} de {PRODUCT_QUIZ_QUESTIONS.length}
          </p>
          <FadeIn key={currentQ}>
            ...
            <div className="space-y-3">
              {q.o.map((opt, i) => (
                <button
                  ...
                  className="w-full flex items-center gap-3 text-left px-5 py-4 rounded-xl border border-white/10 transition-all hover:border-pink-500/50"
      ...
      <section className="min-h-screen flex items-center justify-center px-6 py-24">
        <FadeIn className="max-w-2xl w-full">
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <p className="text-sm" style={{ color: LANDING_COLORS.textMuted }}>Eres un</p>
              ...
              {percentages.map((p) => (
                <div key={p.key}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span style={{ color: PRODUCT_ARCHETYPES[p.key].color }}>{p.pct}%</span>
                  </div>
                  <div className="h-2 rounded-full bg-white/10 overflow-hidden">
                    <div className="h-full rounded-full transition-all duration-700" style={{ width: `${p.pct}%`, backgroundColor: PRODUCT_ARCHETYPES[p.key].color }} />
                  </div>
                </div>
              ))}
            </div>
