...
const AuthCompany: React.FC = () => {
  ...
  const linkUserToCompany = async (userId: string, targetCompanyId: string) => {
    ...
    const { data, error } = await supabase.rpc('link_user_company_by_company_id', {
      company_id: targetCompanyId,
      user_auth_id: userId,
    } as any);

    if (error || !(data as any)?.success) {
      console.error('[AC-4][ERROR] RPC fall� �', error || data);
      throw new Error('No se pudo vincular tu usuario con la empresa.');
    }
  ...
  const handleOAuthLogin = async (provider: "google" | "azure") => {
    ...
    try {
      ...
      toast({
        title: "Error de autenticacion",
        description: error?.message ?? `No se pudo continuar con ${provider}.`,
        variant: "destructive",
      });
      setLoading(false);
    }
